...
window.addEventListener("load", async () => {
  ...
  let treatments = (
    await (
      await fetch(`${window.location.origin}/api/product/getTreatments`)
    ).json()
  ).treatments;
  ...
  function getRowToCopy() {
    //Función que me va a generar la fila que quiero copiar
  ...
  const handleSubstractingQuantity = (e) => {
    //función que se encarga de manera el click del +
    // Agarro al input mas cerca
    const input = e.target.closest("td").querySelector(".quantity-input");
  ...
  function handleInputsQuantity() {
    ...
    let substractQuantityBtns = document.querySelectorAll(
  ...
  function productPrices() {
    ...
    selectElements.each(function () {
      ...
      selectElement.on("change", function () {
        ...
        const price = removeNumberSeparators(
          selectedOption.attributes["data.price"]?.value
        );
        const cashPrice = removeNumberSeparators(
          selectedOption.attributes["data.cash_price"]?.value
        );
  ...
  function checkRowPrices() {
    ...
    rows.forEach((row) => {        
      let singlePrice = parseFloat(
        removeNumberSeparators(row.querySelector(".single-price").innerHTML)
      );
      
      let singleCashPrice = parseFloat(
        removeNumberSeparators(
          row.querySelector(".single-cash-price").innerHTML
        )
      );
      ...
      if (discountInput.value > 0) {
        let discountToAplly = 1- (parseFloat(discountInput.value)/100)
        discountInput.value = parseFloat(discountInput.value);
        //Si hay descuento
        //Para el total: quantity * singlePrice * descuento
        totalCash.innerHTML =
         ( quantity * singlePrice * discountToAplly).toFixed(2);
        // Para el single: singlePrice * descuento
        row.querySelector(".single-cash-price").innerHTML =
          (singlePrice * discountToAplly).toFixed(2);
        
      } else {
        // Tengo que volver a buscar el precio en efectivo para pintarlo
        let select = row.querySelector(".description");
        const selectedOption = select.selectedOptions[0];
        const cashPrice = parseInt(
          removeNumberSeparators(
            selectedOption.attributes["data.cash_price"]?.value
          )
        );
  ...
  function getTotalPrice() {
    ...
    if (discount > 0) {
      ...
      cashTotals.forEach((cashTotal) => {
        cashTotalamount +=
          parseInt(removeNumberSeparators(cashTotal.innerHTML)) || 0;
      });
  ...
  const applyDiscount = (num) => {
    //Aplica el descuento, saca las columnas de pago en efectivo
    ...
    input?.addEventListener("input", function (e) {
      ...
      if (!isNumber(inputValue)) {
        ...
        if (input.classList.contains("discount-input")) {
          //Agarro al discount y le pregunto por el valor
  ...
  PDFGeneratorButton?.addEventListener("click", async () => {
    ...
    bodyClone
      .querySelectorAll(".remove-element")
    ...
    productSelects.forEach((select) => {
      ...
      let selectedOption = select
        .closest(".select-column")
        .querySelector(".select2-selection__rendered").innerHTML;
      const paragraph = document.createElement("p");
    ...
    // si tiene descuento...
    if (discountInput.value > 0) {
      //Agrego esto y borro las columnas cash
      disclaimerDiv.innerHTML = `<p class="disclaimer-p">${discountInput.value}% de descuento aplicado</p>   `;
    }
    disclaimerDiv.innerHTML += `
                    
                    <p class="disclaimer-p">Presupuesto Valido por 7 días</p>
                    <p class="disclaimer-p">Valores con tarjeta de débito y crédito (hasta 3 cuotas).</p>
                    <p class="disclaimer-p">Transferencias bancarias 10% de recargo sobre valor efectivo.</p>
                    ...
                    <div class="disclaimer-details-container">
                        ...
                        <p class="disclaimer-detail">Av.Santa Fe 2911 3 "F"<br>Palermo, Bs.As.</p>
                    </div>
        `;
    ...
    let inputsQuantityContainers = bodyClone.querySelectorAll(
      ".relative-container"
    );
