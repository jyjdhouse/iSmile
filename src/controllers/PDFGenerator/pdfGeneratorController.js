...
const controller = {
  medicalPDF: async (req, res) => {
    ...
    const browser = await setPuppeteerLaunchBrowser();
    ...
    const headerCssFilePath = path.resolve(
      __dirname,
  ...
  budgetPDF: async (req, res) => {
    try {
      ...
      // Envía el PDF como respuesta para descargarlo
      res.setHeader("Content-Type", "application/pdf");
      res.setHeader("Content-Disposition", "inline; filename=formulario.pdf");
      res.send(pdfBuffer);
    } catch (error) {
  ...
  consentPDF: async (req, res) => {
    try {
      ...
      // TEMA FIRMA
      if (req.file) {
        console.log("✅ Firma recibida:", req.file.mimetype, req.file.buffer.length);
        let signature = req.file.buffer;
        const base64Image = signature.toString("base64");
        signaturePath = `data:${req.file.mimetype};base64,${base64Image}`;
      }
      ...
      res.setHeader(
        "Content-Disposition",
        "attachment; filename=Consentimiento.pdf"
      );
      res.send(pdfBuffer);
    } catch (error) {
...
async function setPuppeteerLaunchBrowser() {
  try {
