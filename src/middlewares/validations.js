...
module.exports = {
  userRegistValidations: [
    ...
    body(["email", "password", "re-password"])
      .notEmpty()
      .withMessage("Debes completar el/los campo/s")
      .bail()
      .custom((value, { req }) => {
  ...
  userUpdateValidations: [
    ...
    body(["first_name", "last_name", "phone", "dni"])
      .notEmpty()
      .withMessage("Debes completar el/los campo/s")
      .bail(),
  ...
  orderIsCompleteValidations: [
    ...
    body(["phone", "dni", "billing_zip_code", "shipping_zip_code"])
      .optional()
      .custom((value, { req }) => {
        /*regEX de phone number. Acepta Todo tipo de numero de telefono */
    ...
    body("order_types_id").custom((value, { req }) => {
      ...
      if (!typeInDB) {
        //Quiere decir que no es ningun type aceptado en la db
        throw new Error(msg);
      }
      return true;
    }),
    ...
    body("shipping_province").custom((value, { req }) => {
      ...
      let provinceInDB = value
        ? provincesDB.find((prov) => prov.id == value)
        : null;
      let orderType = req.body.order_types_id;
      let useUserAddress = req.body.use_user_address;
      //Si es envio, no usan misma direccion y esta vacio doy error
    ...
    body("payment_methods_id").custom((value, { req }) => {
      let methodId = req.body.payment_methods_id;
      let paymentMethodInDB = paymentMethods.find(
        (payMeth) => payMeth.id == methodId
      );
